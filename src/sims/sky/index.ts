 ...
 * 再描画をフレームに 1 回へ束ねる。
 *
 * 1 回の描画は帯の 363 方向ぶんの球殻の行進を伴い、実測で 40ms ほどかかる。
 * `input` イベントごとに同期で走らせると、スライダーをドラッグしている間に
 * 描画がキューへ積み上がって **入力に対して際限なく遅れていく**
 * (実測: input を 20 連射すると 2.3 秒ぶんの描画が積まれる)。
 * フレームに 1 回へ潰せば、遅れは常に高々 1 描画ぶんに収まる。
 *
...
export function unmount(): void {
  // 予約したまま破棄すると、破棄済みの帯や図に対してコールバックが走る。
  if (renderHandle !== 0) {
